...
app = FastAPI(
    ...
    description=(
        "API for HomeQuestAI: a smart real estate platform for Chennai. "
        "Features: user management, properties, recommendations, chat, "
...
@app.get(
    ...
    description="Checks DB connection and returns status",
)
def db_health_check():
    """
...
def create_property(
    ...
    listed_by: int = Query(..., description="User ID of listing agent"),
    db: Session = Depends(get_db),
):
...
def get_user_viewings(user_id: int, db: Session = Depends(get_db)):
    ...
    viewings = (
        db.query(Viewing)
        .filter(Viewing.user_id == user_id)
...
@app.get(
    ...
    tags=["VirtualTours"],
    summary="Get property tour media",
)
